    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] p-4">
      <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10 border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <input type="email" placeholder="E-mail" className="w-full px-6 py-4 rounded-2xl border border-slate-200 focus:ring-2 focus:ring-indigo-600 outline-none transition-all" value={email} onChange={e => setEmail(e.target.value)} required />
          <input type="password" placeholder="Wachtwoord" className="w-full px-6 py-4 rounded-2xl border border-slate-200 focus:ring-2 focus:ring-indigo-600 outline-none transition-all" value={password} onChange={e => setPassword(e.target.value)} required />
          {error && <p className="text-red-500 text-xs font-bold text-center">Onjuiste gegevens.</p>}
...
const UserView: React.FC<{ 
  ...
  currentUser: User
}> = ({ tasks, selectedDate, setSelectedDate, onToggle, isCompleted, currentUser }) => {
...
}> = ({ tasks, users, onAddTask, onDeleteTask }) => {
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in duration-200">
            ...
            <form onSubmit={(e) => { e.preventDefault(); onAddTask(formData); setIsAdding(false); setFormData({title:'', description:'', frequency:TaskFrequency.DAILY}); }} className="space-y-4">
              <input type="text" placeholder="Titel" className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50 outline-none" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} required />
              <textarea placeholder="Beschrijving" className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50 outline-none" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
              <select className="w-full px-6 py-4 rounded-2xl border border-slate-100 bg-slate-50 outline-none font-bold" value={formData.frequency} onChange={e => setFormData({...formData, frequency: e.target.value as TaskFrequency})}>
...
const App: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (profiles && profiles.length > 0) {
          setUsers([...profiles.map(p => ({ ...p, role: p.role as UserRole })), ...INITIAL_USERS]);
        }
        const { data: dbTasks } = await supabase.from('tasks').select('*');
        if (dbTasks) setTasks(dbTasks.map(t => ({...t, assignedTo: t.assigned_to, frequency: t.frequency as TaskFrequency})));
  ...
  const addTask = async (newTask: any) => {
    const task: Task = { ...newTask, id: Math.random().toString(36).substr(2, 9), createdAt: new Date().toISOString(), startDate: formatDateISO(new Date()), assignedTo: 'user-3' };
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f8fafc]">
      <nav className="w-full md:w-80 bg-white border-r border-slate-100 flex flex-col p-8 gap-10">
        ...
        <div className="flex flex-col gap-2">
          <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2">Menu</p>
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto">
        <div className="max-w-4xl mx-auto">
          {viewMode === 'daily' && <UserView tasks={tasks.filter(t => t.assignedTo === currentUser.id || t.assignedTo === 'user-3')} selectedDate={selectedDate} setSelectedDate={setSelectedDate} onToggle={toggleTask} isCompleted={(tid, d) => completions.some(c => c.taskId === tid && c.completedAt === d)} currentUser={currentUser} />}
          {viewMode === 'manage' && currentUser.role === UserRole.ADMIN && <AdminDashboard tasks={tasks} users={users} onAddTask={addTask} onDeleteTask={removeTask} />}
        </div>
      </main>
    </div>
  );
